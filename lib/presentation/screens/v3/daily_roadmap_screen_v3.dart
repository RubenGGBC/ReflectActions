...
class _DailyRoadmapScreenV3State extends State<DailyRoadmapScreenV3>
          ...
          return AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              final animationValue = Curves.easeOutCubic.transform(
                (_staggerAnimation.value - delay).clamp(0.0, 1.0) / (1.0 - delay).clamp(0.001, 1.0),
              );
  ...
  void _showAddActivityModal(DailyRoadmapProvider provider, int hour, int minute) {
    showModalBottomSheet(
      ...
      // Trigger bounce animation for new activities
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
  ...
  bool _isCurrentHour(int hour) {
    ...
    // Solo es la hora actual si es hoy y la hora coincide
    final isToday = selectedDate.year == now.year &&
                   selectedDate.month == now.month &&
                   selectedDate.day == now.day;
    
    return isToday && now.hour == hour;
  }
  ...
  bool _isPastHour(int hour) {
    ...
    // Si la fecha seleccionada es anterior a hoy, todas las horas son pasadas
    if (selectedDate.isBefore(DateTime(now.year, now.month, now.day))) {
      return true;
    }
    
    // Si la fecha seleccionada es posterior a hoy, ninguna hora es pasada
    if (selectedDate.isAfter(DateTime(now.year, now.month, now.day))) {
      return false;
    }
    
    // Si es el día actual, comparar la hora
