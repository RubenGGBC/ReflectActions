...
class OptimizedDatabaseService {
  ...
  Future<void> _migrateToV15(Database db) async {
    try {
      ...
      if (!hasColorHex) {
        ...
        _logger.i('✅ Columna color_hex agregada a la tabla tags');
      } else {
  ...
  Future<int?> saveInteractiveMoment(int userId, OptimizedInteractiveMomentModel moment) async {
    try {
      ...
      return await db.transaction((txn) async {
        ...
        _logger.d('💾 Insertando momento: $momentData');
  ...
  List<String> _generatePatternInsights(List<Map<String, dynamic>> weeklyPatterns) {
    ...
    // Filter out entries with null avg_mood values
    final validPatterns = weeklyPatterns.where((pattern) => pattern['avg_mood'] != null).toList();
    
    if (validPatterns.isEmpty) return insights;
    
    final bestDay = validPatterns.reduce((a, b) => (a['avg_mood'] as double? ?? 0.0) > (b['avg_mood'] as double? ?? 0.0) ? a : b);
    final worstDay = validPatterns.reduce((a, b) => (a['avg_mood'] as double? ?? 0.0) < (b['avg_mood'] as double? ?? 0.0) ? a : b);
  ...
  Future<bool> updateGoalStatus(int goalId, GoalStatus status) async {
    try {
      ...
      _logger.i('✅ BD: Estado actualizado - goalId: $goalId, status: ${status.name}, rowsAffected: $rowsAffected');
      return rowsAffected > 0;
    } catch (e) {
      _logger.e('❌ Error actualizando estado del goal: $e');
      return false;
    }
  }

  /// Establece la fecha de completación de un objetivo.
